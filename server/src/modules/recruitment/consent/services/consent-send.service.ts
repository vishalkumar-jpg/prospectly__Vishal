...
@Injectable()
export class ConsentSendService {
  private readonly logger = new Logger(ConsentSendService.name);
  ...
  async sendConsent(userId: string, matchId: string) {
    ...
    if (match.status !== JOB_POOL_MATCH_STATUS.PENDING) {
      throw new BadRequestException(
        CONSENT_MESSAGES.ERROR.CONSENT_ALREADY_SENT
      );
    }
    ...
    // Rule 3: lock by email (not just contactId — duplicate contacts possible).
    if (
    ...
    // Rule 2: permanent global block for this connector + candidate.
    if (
    ...
    if (!emailResult.success) {
      this.logger.error(
        `CONSENT_SEND_SERVICE :: SEND_CONSENT : EMAIL_FAILED : ${emailResult.error}`
      );
    } else {
      try {
        ...
        this.logger.error(
          `CONSENT_SEND_SERVICE :: SEND_CONSENT : EMAIL_LOG : ERROR : ${logError}`
        );
      }
    }

    return {
      message: CONSENT_MESSAGES.SUCCESS.CONSENT_SENT,
      candidateName,
    };
  }
}
