...
const SIDEBAR_COOKIE_NAME = COOKIE_PREFIX
  ? `${COOKIE_PREFIX}_sidebar:state`
  : "sidebar:state";
...
>(
  (
    ...
    // Read the initial state from cookie, falling back to defaultOpen if cookie doesn't exist
    // Use lazy initializer to read cookie only once on mount
    const [_open, _setOpen] = React.useState(() => {
      ...
      // Fall back to defaultOpen prop if no cookie exists
      return defaultOpen;
    });
      ...
      return (
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          <SheetContent
            ...
            data-mobile="true"
            className="w-[--sidebar-width] sm:max-w-none bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden"
            style={
              {
