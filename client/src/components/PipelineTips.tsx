...
import { Crown, Star, X, ChevronDown } from "lucide-react";

interface PipelineTipsProps {
  type: "making" | "requesting";
}

const PipelineTips = ({ type }: PipelineTipsProps) => {
  const [showTips, setShowTips] = useState(true);

  // Load tips visibility from localStorage on mount
  useEffect(() => {
    const stored = localStorage.getItem("pipeline-tips-visible");
    if (stored !== null) {
  ...
  const toggleTips = (visible: boolean) => {
    setShowTips(visible);
    localStorage.setItem("pipeline-tips-visible", JSON.stringify(visible));
  };
  ...
  const makingContent = {
    badge: "Earn $2,500+ Monthly",
    title: "Maximize Your Introduction Earnings",
    subtitle:
      "Turn your network into consistent income by helping others connect",
    stats: [
      // {
      //   value: "4.2x",
      //   label: "Higher Earnings",
      //   desc: "vs. one-time bounties",
      // },
      // {
      //   value: "$385",
      //   label: "Avg Per Introduction",
      //   desc: "completed successfully",
      // },
      // {
      //   value: "87%",
      //   label: "Success Rate",
      //   desc: "for active pipeline users",
      // },
      // { value: "2.5h", label: "Avg Response Time", desc: "for top earners" },
    ],
    testimonials: [
      // {
      //   text: "I turned my LinkedIn network into $4K monthly income just by making 2-3 quality introductions per week.",
      //   author: "Alex Chen",
      //   role: "VP Sales • $28K earned in 6 months",
      // },
      // {
      //   text: "The pipeline view helps me track 15+ introductions simultaneously. My success rate jumped to 92%.",
      //   author: "Maria Santos",
      //   role: "Business Development • Top 1% earner",
      // },
    ],
  };

  const requestingContent = {
    badge: "Get Quality Connections",
    title: "Accelerate Business Growth",
    subtitle:
      "Access warm introductions to decision-makers through trusted connections",
    stats: [
      {
        value: "5.8x",
        label: "Higher Response Rate",
        desc: "vs. cold outreach",
      },
      {
        value: "73%",
        label: "Meeting Success Rate",
        desc: "from warm introductions",
      },
      {
        value: "2.3 days",
        label: "Avg Response Time",
        desc: "for introduction requests",
      },
      {
        value: "$150K",
        label: "Avg Deal Value",
        desc: "through warm connections",
      },
    ],
    testimonials: [
      {
      ...
      {
        text: "Request pipeline helped me track 12 simultaneous introductions. My conversion rate went from 15% to 68%.",
  ...
  const content = type === "making" ? makingContent : requestingContent;
